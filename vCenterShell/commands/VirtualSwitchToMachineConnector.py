...
class VirtualSwitchToMachineConnector(object):
    ...
    def connect(self, vcenter_name, dv_switch_path, dv_switch_name, dv_port_name, vm_uuid, port_group_path,
                vlad_id, vlan_spec):
        si, vm = self.get_vm(vcenter_name, vm_uuid)
    ...
    def connect_networks(self, vcenter_name, vm_uuid, vlan_spec, mapping):
        ...
        for vnic_name, network_spec in mapping.items():
            network = self.get_or_create_network(si,
                                                 ...
                                                 network_spec['vlad_id'],
                                                 vlan_spec)
            networks_to_connect.append(network)

        self.virtual_machine_port_group_configurer.connect_by_mapping(vm, networks_to_connect)
